  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        ...
        const bestSellingRes = await productApi.getProducts({ sort_by: "sold", order: "desc", limit: 10 });
        setBestSellingProducts(bestSellingRes.data.products);

        // Lấy 10 sản phẩm xem nhiều nhất
        const mostViewedRes = await productApi.getProducts({ sort_by: "views", order: "desc", limit: 10 });
        setMostViewedProducts(mostViewedRes.data.products);
  ...
  return (
    <div className="w-full pb-20">
      ...
      <div className="max-w-7xl mx-auto px-6 flex flex-col gap-24">
        ...
        <section>
          <div className="flex justify-between items-end mb-8 border-b-4 border-black pb-4">
            <h2 className="text-3xl font-serif font-bold uppercase tracking-widest">Xem Nhiều Nhất</h2>
            <Link to="/shop?sort_by=views&order=desc" className="font-bold underline hover:text-gray-600 transition-colors">
              Xem tất cả
            </Link>
          </div>
          <Swiper
